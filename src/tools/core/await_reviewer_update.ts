    ...
    handler: async (data: any) => {
        ...
        return new Promise((resolve) => {
            ...
            timeoutId = setTimeout(() => {
                eventBus.off(`${data.taskId}.post_rejection`, listener);
                eventBus.off(`${data.taskId}.post_approval`, listener);
                resolve({ content: [{ type: 'text', text: `Wait timed out after ${timeoutSeconds} seconds. The Reviewer might be still working or stuck. You can re-call this tool to keep waiting, or take other appropriate actions.` }] });
            }, AWAIT_TIMEOUT_MS);
        });
    }
};
