...
import { render as engineRender, Context } from '@tsomaiatech/moxite'
...
export class TemplateManager {
    ...
    /**
     * Copies all default templates to the project's `.relay/templates/` if they don't exist.
     * Does not overwrite existing user templates (preserves customizations).
     * Call during `relay init` to populate the template directory.
     */
    public copyAllDefaultsToProject(projectRoot: string): void {
        const userDir = path.join(projectRoot, '.relay', 'templates')
        if (!fs.existsSync(userDir)) {
            fs.mkdirSync(userDir, { recursive: true })
            console.log(`[TemplateManager] Scaffolding local template directory: ${userDir}`)
        }

        const files = fs.readdirSync(this.defaultTemplateDir)
        for (const filename of files) {
            const defaultPath = path.join(this.defaultTemplateDir, filename)
            const userPath = path.join(userDir, filename)
            if (fs.statSync(defaultPath).isFile() && !fs.existsSync(userPath)) {
